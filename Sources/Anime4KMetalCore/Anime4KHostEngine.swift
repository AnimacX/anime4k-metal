...
public final class Anime4KHostEngine: @unchecked Sendable {
    ...
    private struct Anime4KPipeline {
        ...
        func purgeTextureResources() {
            workspace.purge()
            stages.forEach { $0.purgeTextureResources() }
        }

        var textureResourceSnapshot: (cachedTextureCount: Int, allocationCount: Int) {
            let stageSnapshot = stages.reduce(into: (cachedTextureCount: 0, allocationCount: 0)) {
                $0.cachedTextureCount += $1.cachedTextureCount
                $0.allocationCount += $1.outputTextureAllocationCount
            }
            return (
                workspace.cachedTextureCount + stageSnapshot.cachedTextureCount,
                workspace.allocationCount + stageSnapshot.allocationCount
            )
        }
    }
    ...
    public init(preferredDevice: MTLDevice? = nil) throws {
        ...
        public func debugSnapshot() -> Anime4KHostEngineDebugSnapshot {
            ...
            let outputTextureSnapshot = pipelines.values.reduce(
                into: (cachedTextureCount: 0, allocationCount: 0)
            ) {
                let snapshot = $1.textureResourceSnapshot
                $0.cachedTextureCount += snapshot.cachedTextureCount
                $0.allocationCount += snapshot.allocationCount
            }
    ...
    /// 手动释放 Anime4K pipeline 缓存的输出与工作纹理，并保留已编译的 shader pipeline。
    /// 此方法会等待正在进行的增强完成。
    public func purgeOutputTextureCache() {
        ...
        stateLock.unlock()
        activePipelines.forEach { $0.purgeTextureResources() }
    }
...
private final class Anime4KSharedWorkspace {
    ...
    var cachedTextureCount: Int {
        textures.values.reduce(0) { $0 + $1.count }
    }
...
private final class Anime4KProcessor {
    ...
    private var frameOutputTextureAllocationCount = 0
    ...
    var cachedTextureCount: Int {
        frameOutputTexture == nil ? 0 : 1
    }

    var outputTextureAllocationCount: Int {
        frameOutputTextureAllocationCount
    }
        ...
        -> MTLTexture
    {
        ...
        if direct == nil, frameOutputTexture == nil {
            ...
            if frameOutputTexture != nil {
                frameOutputTextureAllocationCount += 1
            }
        }
